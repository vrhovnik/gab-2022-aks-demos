...
var namespaceToList = Environment.GetEnvironmentVariable("NamespaceToList") ?? "default";

HorizontalRule($"List pods in namespace - {namespaceToList}");
var listPodInNamespace = await client.ListNamespacedPodAsync(namespaceToList);
...
var namespaceNameForTest = Environment.GetEnvironmentVariable("TestNamespace") ?? "test";
...
AnsiConsole.WriteLine(str); //ouput ls command
...
foreach (var obj in objects) AnsiConsole.WriteLine(obj.ToString() ?? "No object defined");
...
using (podlistResp.Watch<V1Pod, V1PodList>((type, item) =>
       {
           AnsiConsole.WriteLine("==on watch event==");
